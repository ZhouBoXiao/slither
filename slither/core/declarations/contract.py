...
class Contract(ChildSlither, SourceMapping):
    ...
    def get_summary(self):
        ...
        return (self.name, [str(x) for x in self.inheritance], [str(x) for x in self.variables], func_summaries,
                modif_summaries)
    ...
    def __str__(self):
        # return self.name
        res = self.contract_kind

        if len(self.immediate_inheritance) > 0:
            res += " %s is %s {\n\t" % (self.name, ",".join(map(lambda x: "%s" % x.name,
                                                                self.immediate_inheritance)))
        else:
            res += " %s {\n\t" % self.name
        # if self.enums is not None:
        if len(self.using_for) > 0:
            for (key, value) in self.using_for.items():
                if key == '*':
                    res += 'using {0} for *;\n\t'.format(str(value[0]))
                else:
                    res += 'using {0} for {1};\n\t'.format(str(value[0]), str(value[0]) + '.' + str(key))
        if len(self.state_variables_declared) > 0:
            res += "\n\t".join(map(lambda x: "%s" % str(x), self.state_variables_declared))
            res += "\n\t"
        if len(self.enums_declared) > 0:
            res += "\n\t".join(map(lambda x: "%s" % str(x), self.enums_declared))
            res += "\n\t"
        if len(self.events_declared) > 0:
            res += "\n\t".join(map(lambda x: "%s" % str(x), self.events_declared))
            res += "\n\t"
        if len(self.structures_declared) > 0:
            res += "\n\t".join(map(lambda x: "%s" % str(x), self.structures_declared))
            res += "\n\t"
        if len(self.functions_and_modifiers_declared) > 0:
            res += "\n\t".join(map(lambda x: "%s" % str(x),
                                   [func for func in self.functions_and_modifiers_declared
                                    if type(func).__name__ in ('FunctionSolc', 'ModifierSolc')]))
        res += "\n}"
        return res
    # endregion
