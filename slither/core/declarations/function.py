...
class Function(ChildContract, ChildInheritance, SourceMapping):
    ...
    def full_name(self):
        ...
        return name+'('+','.join(map(lambda x: "%s" % str(x)[:-1], self._parameters))+')'
    ...
    def __str__(self):
        ...
        if self.name == 'constructor':
            ...
            if self.visibility is not None:
                res += " " + self.visibility + " "
        elif type(self).__name__ == 'ModifierSolc':
            res = "modifier " + self.full_name
        else:
            res = "function " + self.full_name
            if self.visibility is not None:
                res += " " + self.visibility + " "
        if self.payable:
            res += "payable "
        if len(self.modifiers) > 0:
            res += " ".join(map(lambda x: "%s;" % str(x), self.modifiers))
        if self.view is not None:
            res += "view "
        if self.pure is not None:
            res += "pure "
        if len(self.returns) > 0:
            res += 'returns(' + ",".join(map(lambda x: "%s" % str(x)[:-1], self.returns)) + ')'

        if len(self.nodes) > 0:
            res += "{\n\t\t"
            self._str = ""
            self.get_son(self.nodes)
            # print( self._str)
            res += self._str
            # for node in self.nodes:
            #     if str(node) != "None":
    ...
    def get_son(self, nodes):
        for i, son in enumerate(nodes):
            if son not in nodes[i-1].sons and str(son) != "None":
                self._str += '} else {\n\t\t'
            if son.type == 0:
                continue
            elif son.type == 0x40:
                ...
                self._str += str(son.variable_declaration) + '\n\t\t'
            elif son.type == 0x12:   #IF
                self._str += 'if ({0})'.format(str(son))
                if len(son.sons) > 0:
                    self._str += '{\n\t\t'
            elif son.type == 0x11:   #return
                self._str += 'return {0};\n\t\t'.format(str(son))
            elif son.type == 0x50:  # ENDIF
                self._str += '}\n\t\t'
            elif son.type == 0x10 or son.type == 0x13:
                self._str += str(son) + ';\n\t\t'
            elif son.type == 0x51:
                self._str += '\twhile ({0})'.format(str(son))
                if len(son.sons) > 0:
                    self._str += '{\n\t\t'
            elif son.type == 0x52:
                self._str += '}\n\t'
